    ...
    <div className="animate-fade-in flex flex-col min-h-screen pb-32">
      ...
      <header className="pt-8 pb-5 px-6 flex flex-col items-center text-center">
        <div className="w-full flex justify-between items-center absolute top-5 left-0 px-6">
          <button 
            onClick={onBack}
            className="flex size-9 items-center justify-center rounded-[10px] bg-white/5 border border-white/10 active:scale-90 transition-transform"
          >
            <span className="material-symbols-outlined text-white text-lg">arrow_back</span>
          </button>
          <button className="flex size-9 items-center justify-center rounded-[10px] bg-white/5 border border-white/10 active:scale-90 transition-transform">
      ...
      {/* Main Action Area - Radius 24px -> 20px, internal 22.5px -> 19px */}
      <div className="px-6 flex flex-col gap-5">
        <button 
          onClick={onToggle}
          className={`relative w-full aspect-[16/10] rounded-[20px] p-[1.2px] transition-all duration-500 overflow-hidden shadow-2xl active:scale-[0.97] ${
            isCompleted 
        ...
        >
          ...
          <div className={`relative h-full w-full rounded-[19px] flex flex-col items-center justify-center gap-3 transition-all duration-500 ${
        ...
        {/* Stats Grid - Radius 24px -> 20px */}
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-surface/40 rounded-[20px] p-4 border border-white/5 flex flex-col gap-1 relative overflow-hidden group">
